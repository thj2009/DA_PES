...
xfull, yfull, xdev_full, dydx_full = extract(fulllist, origin[0], shift=False, scale=scale)

xfull = xfull[:, [3,4,6,7,9,10,12,13]]
dydx_full = np.array(dydx_full)[:, [3,4,6,7,9,10,12,13]]
yfull = np.array(yfull) - origin[0]['E0']
...
#pce = DA_PES.PCE(2, 3, withdev=True, copy_xy=True, regr='OLS', polytype='Herm')
pce = DA_PES.PCE(8, 2, withdev=True, copy_xy=True, regr='OLS', polytype='Herm')

#xtrain, xtest, ytrain, ytest, xdev_train, xdev_test, dydx_train, dydx_test = \
#        train_test_split(x_t, yfull, x_t, dydx_t, test_size=5, random_state=np.random.randint(0,3000))

train_size = 5
xtrain, xtest, ytrain, ytest, xdev_train, xdev_test, dydx_train, dydx_test = \
        train_test_split(xfull, yfull, xfull, dydx_full, test_size=train_size,
...
#    inte = np.sum(np.exp((-ypred_sample[:,i] * _conv.eV_2_J)/(_const.kB * Tem))) / (nsample) 
    inte = np.pi / 3. * np.sum(np.exp((-ypred_sample[:,i] * _conv.eV_2_J)/(_const.kB * Tem))) / len(ypred_sample)
...
##
##
np.save('logint_methane_rot_%d.npy' %Tem, log_int)
