...
import os
# Analyst Result
fld = './ronak/result/scale1'
#fld = './ronak/result/scale&weight'

DOF = 16

scls = ['0.045', '0.015', '0.010', '0.005']
#scls = ['0.015']
...
for scl in scls:
    ...
    for train_size in train_sizes:
        file = os.path.join(fld, scl, 'result_%d.txt' %train_size)
        fp = open(file, 'r')
...
for i in range(len(scls)):
    ax.errorbar(train_sizes, rmse_all[i], yerr=std_all[i], label=scls[i],
...
ax = fig.gca()
for i in range(len(scls)):
    ax.errorbar(train_sizes, rmseDOF_all[i], yerr=rmseDOF_std_all[i], label=scls[i],
                fmt=fmts[i], markersize=4, elinewidth=1.5)
# ticks parameter size
ax.tick_params(axis='both', which='minor', direction='in', labelsize=7)
ax.tick_params(axis='both', which='major', direction='in', labelsize=7)
