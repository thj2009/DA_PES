...
print('=='*20)
print('DOF = %d' %DOF)
print('Threshold = %.1f' %thres)
#print('Vibrational Freq = %.2f' %Svib)
print(np.mean(Slist), '+/-', np.std(Slist))
#
np.save('Svib_ethane_thres_%.1f.npy' %thres, Slist)
...
# Neglect wave less than thres

Slist_neg = []
Zfull = []
for i in range(nsample):
    _wave = wave_list[i][:DOF]
    _wave = [w for w in _wave if w > thres]
    eps = np.array(_wave) * 100 * c * h
    beta = 1. / (kb * Tems)
    term = np.exp(-np.outer(beta, eps))
    SS = -np.log(1 - term) + np.outer(beta, eps) / (term**(-1) - 1)
    Svib = np.sum(SS, axis=1)
    Slist_neg.append(Svib)
#
Slist_neg = np.array(Slist_neg)
...
print('Threshold = %.1f' %thres)
#print('Vibrational Freq = %.2f' %Svib)
print(np.mean(Slist_neg), '+/-', np.std(Slist_neg))
#
np.save('Svib_ethane_negthres_%.1f.npy' %thres, Slist_neg)
