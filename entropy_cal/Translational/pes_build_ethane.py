...
xtrain, xtest, ytrain, ytest, xdev_train, xdev_test, dydx_train, dydx_test = \
        train_test_split(xfull, yfull, xfull, dydx_t, test_size=0)
...
Tem = 106 # K
AREA = np.cross(DX[:2], DY[:2])/2. * _conv.A_2_m **2 * scale ** 2
INTE = np.sum(np.exp((-ysample * _conv.eV_2_J)/(_const.kB * Tem))) / (nsample) 
PREFC = 2 * np.pi * _const.kB * Tem / (_const.h ** 2) * (12.011*2 + 6) * _conv.u_2_kg
...
np.save('logint_ethane_2D_%d.npy' %Tem, inteTotSample)
...
print('  UQ: log(INTEGRAL) = %.3e +/-  %.3e' %(np.mean(inteTotSample), np.std(inteTotSample)))


plt.figure()
plt.hist(inteTotSample, bins=50, density=True, label=lb)
plt.hist(log_int, bins=50, density=True, label=lb)
...
#Tot = np.array(Tot)
#plt.figure()
#plt.fill_between(Tems, np.max(Tot, axis=1), np.min(Tot, axis=1), alpha=0.3, color='r')
